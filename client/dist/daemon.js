 ...
 */
export function registerDaemon(instanceId, startCmd) {
    if (trySystemd(instanceId, startCmd))
        return;
    if (tryPm2(instanceId, startCmd))
        return;
    startNohup(instanceId, startCmd);
}
// ── systemd ───────────────────────────────────────────────────────────────
function trySystemd(instanceId, startCmd) {
...
// ── pm2 ───────────────────────────────────────────────────────────────────
function tryPm2(instanceId, startCmd) {
...
// ── nohup fallback ────────────────────────────────────────────────────────
function startNohup(instanceId, startCmd) {
    ...
    const result = spawnSync("/bin/sh", ["-c", `nohup ${startCmd} >> "${logFile}" 2>&1 & echo $!`], { stdio: ["ignore", "pipe", "pipe"] });
